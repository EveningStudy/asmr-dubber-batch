...
输入 `1` 到 `7` 可以只修改对应项目；改完会回到同一个总览，不需要把其他问题重新回答一遍。下一个作品默认沿用上一个作品的处理类型、普通/和谐分支、成品组织、特典和字幕选择。
...
4. 粘贴第一个解压后的作品文件夹路径。
5. 查看默认选项总览。输入 `1` 到 `7` 修改对应项目；不需要修改时直接按 Enter 确认并输入下一个作品路径。
6. 当前作品确认无误后输入 `0`，或者在下一个路径提示处直接按 Enter。所有作品都配置完以后，程序才开始 ASR、翻译和 TTS。
7. 网页打开后，点击“打开项目”，选择一段清晰音频并点击“设为项目音色参考”。如果还改了校对表格，也要保存表格。
8. 回到命令行等待完成。

队列按添加顺序处理。某个作品失败时，它已有的状态和成品会保留，程序会记录错误并继续下一个作品；队列结束时会列出失败项目。再次运行失败的作品可以续跑。按 `Ctrl+C` 会停止整个队列。
